...
static int db_client_call_find_and_update(struct db_client *client,
                                          int db_type,
                                          void *match_partial,
                                          void *new_record,
                                          size_t record_size,
                                          unsigned int match_mask)
{
    ...
    req.msg_type = 1;
    req.reply_to = make_request_id(client);
    req.operation = DB_OP_FIND_AND_UPDATE;
    req.db_id = db_type;
    req.match_mask = match_mask;

    memcpy(&req.payload, match_partial, record_size); // what to match
    memcpy(&req.payload2, new_record, record_size);   // what to write
...
// Orders
int db_client_find_and_update_order(struct db_client *client,
                                    struct orders *match_partial,
                                    unsigned int match_mask,
                                    struct orders *new_record)
{
    return db_client_call_find_and_update(client, 1,
                                          match_partial, new_record,
                                          sizeof(struct orders), match_mask);
}

// Menu
int db_client_find_and_update_menu(struct db_client *client,
                                   struct menu *match_partial,
                                   unsigned int match_mask,
                                   struct menu *new_record)
{
    return db_client_call_find_and_update(client, 2,
                                          match_partial, new_record,
                                          sizeof(struct menu), match_mask);
}

// Tables
int db_client_find_and_update_table(struct db_client *client,
                                    struct tables *match_partial,
                                    unsigned int match_mask,
                                    struct tables *new_record)
{
    return db_client_call_find_and_update(client, 3,
                                          match_partial, new_record,
                                          sizeof(struct tables), match_mask);
}

// Users
int db_client_find_and_update_user(struct db_client *client,
                                   struct users *match_partial,
                                   unsigned int match_mask,
                                   struct users *new_record)
{
    return db_client_call_find_and_update(client, 4,
                                          match_partial, new_record, sizeof(struct users), match_mask);
}